...
export async function runSeed(force=false){
  ...
  if(force||recSnap.empty){
    ...
    const recipes=[
      {name:'Cheeseburguer Estradeiros (bovino)',description:'Brioche, blend bovino 160g, queijo prato, maionese da casa.',items:[
      ...
      {name:'Cheddar Bacon',description:'Brioche, blend bovino 160g, cheddar e bacon crocante.',items:[
        {ingredientId:by('Pão brioche 80g')?.id,ingredientName:'Pão brioche 80g',qty:1,unit:'un',cost:by('Pão brioche 80g')?.unitCost||0},
        {ingredientId:by('Blend bovino 160g')?.id,ingredientName:'Blend bovino 160g',qty:1,unit:'un',cost:by('Blend bovino 160g')?.unitCost||0},
      ...
      {name:'Clássico Suíno 160g',description:'Blend suíno (toscana + filé mignon suíno), brioche, queijo prato e maionese.',items:[
        {ingredientId:by('Pão brioche 80g')?.id,ingredientName:'Pão brioche 80g',qty:1,unit:'un',cost:by('Pão brioche 80g')?.unitCost||0},
      ...
      {name:'Veggie da Estrada',description:'Hambúrguer de soja com beterraba 120g, brioche, cheddar e picles.',items:[
        {ingredientId:by('Pão brioche 80g')?.id,ingredientName:'Pão brioche 80g',qty:1,unit:'un',cost:by('Pão brioche 80g')?.unitCost||0},
...
(async()=>{ try{ await runSeed(false);}catch(e){} })();
